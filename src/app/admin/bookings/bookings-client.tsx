...
export type AdminBookingItem = {
  ...
  /** Set while a price the provider proposed is waiting on the customer. */
...
function Row({ booking }: { booking: AdminBookingItem }) {
  ...
  return <tr>
    ...
    <td className="p-4"><span className="rounded-full bg-muted px-2 py-1 text-xs font-bold">{booking.status.replaceAll("_", " ")}</span>{booking.counterAmount != null && <p className="mt-1 text-xs text-primary">GH₵{booking.counterAmount.toLocaleString()} proposed · awaiting customer</p>}</td>
    <td className="p-4"><span className="flex items-center gap-1 text-xs font-semibold"><CreditCard className="h-3.5 w-3.5" />{booking.paymentStatus.replaceAll("_", " ")}</span>{booking.amount ? <p className="mt-1 text-xs">GH₵{booking.amount.toLocaleString()}</p> : booking.depositAmount ? <p className="mt-1 text-xs text-primary">GH₵{booking.depositAmount.toLocaleString()} <span className="text-muted-foreground">(held)</span></p> : null}</td>
