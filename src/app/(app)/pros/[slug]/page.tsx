...
export default async function ProviderProfilePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const provider = await prisma.provider.findFirst({
    ...
    include: {
      ...
      services: { where: { active: true }, orderBy: { createdAt: "desc" } },
      portfolio: { orderBy: { sortOrder: "asc" } },
      savedBy: { where: { customerId: user.id }, select: { id: true } },
  ...
  const prices = provider.services.map((s) => s.startingPrice).filter((p): p is number => p != null)
  const startingPrice = prices.length ? Math.min(...prices) : null
  const isSaved = provider.savedBy.length > 0

  return (
    <main className="min-h-screen bg-muted/20 pb-12">
      <div className="mx-auto max-w-6xl px-4 py-6 sm:px-6">
        <div className="flex items-center justify-between gap-4">
          <Link href="/search" className="inline-flex items-center gap-1 text-sm font-semibold text-muted-foreground hover:text-foreground">
            <ArrowLeft className="h-4 w-4" />
            Back to search
          </Link>
          {/* Action bar Save Button */}
          <div className="flex items-center gap-2">
            <SaveButton providerId={provider.id} initialSaved={isSaved} />
          </div>
        </div>

        {/* Hero Section */}
        <section className="mt-5 overflow-hidden rounded-3xl border border-border bg-card shadow-sm">
          <div className="relative h-60 bg-accent sm:h-72">
            {provider.coverImageUrl ? (
              <Image
                src={provider.coverImageUrl}
                alt={provider.storeName ?? provider.user.name}
                fill
                priority
                sizes="1152px"
              ...
              <Icon className="absolute inset-0 m-auto h-20 w-20 text-primary/20" />
            ) : null}
            ...
            <div className="absolute bottom-6 left-6 right-6 flex flex-wrap items-end justify-between gap-4 text-white">
              <div>
                <div className="flex items-center gap-2">
                  <h1 className="text-2xl font-extrabold sm:text-3xl">{provider.storeName ?? provider.user.name}</h1>
                  <BadgeCheck className="h-6 w-6 text-emerald-300 shrink-0" />
                </div>
                <p className="mt-1 text-sm text-white/80">
                  {category?.label} · {provider.locationLabel}
                </p>
              </div>

              {/* Cover Hero Save Button */}
          ...
          <div className="grid grid-cols-2 divide-x divide-border sm:grid-cols-4">
            {[
              ["Rating", provider.totalReviews ? provider.avgRating.toFixed(1) : "New"],
              ["Reviews", provider.totalReviews],
              ["Jobs done", provider._count.bookings],
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_380px]">
          <div className="space-y-6">
            <section className="rounded-3xl border border-border bg-card p-6">
              <p className="text-xs font-bold uppercase text-primary">About</p>
              <h2 className="mt-2 text-xl font-extrabold">Verified professional</h2>
              <p className="mt-4 text-sm leading-7 text-muted-foreground">{provider.bio}</p>
              <div className="mt-5 flex items-center gap-2 rounded-2xl bg-muted/40 p-4 text-sm">
                <ShieldCheck className="h-5 w-5 text-primary shrink-0" />
                Identity reviewed and reviews linked to completed bookings.
              </div>
            </section>
                  ...
                  {provider.services.map((service) => (
                    <div key={service.id} className="rounded-2xl border border-border p-4">
                      ...
                      {service.startingPrice && (
                        <p className="mt-3 text-sm font-bold text-primary">From GH₵{service.startingPrice.toLocaleString()}</p>
                      )}
                    </div>
                  ))}
                </div>
            ...
            {provider.portfolio.length > 0 && (
              <section className="rounded-3xl border border-border bg-card p-6">
                <h2 className="text-xl font-extrabold">Recent work</h2>
            ...
            <section className="rounded-3xl border border-border bg-card p-6">
              <div className="flex items-center justify-between">
                <h2 className="text-xl font-extrabold">Verified reviews</h2>
              ...
              <div className="mt-4 grid gap-3 sm:grid-cols-2">
                {provider.reviews.map((review) => (
                  <article key={review.id} className="rounded-2xl bg-muted/30 p-4">
                    ...
                    <p className="mt-3 text-sm">{review.comment || "Great service experience."}</p>
                    <p className="mt-3 text-xs font-semibold text-muted-foreground">{review.customer.name} · Verified booking</p>
                  </article>
                ))}
              </div>
            </section>
          ...
          <aside className="lg:sticky lg:top-24">
            <BookingPanel providerId={provider.id} providerName={provider.storeName ?? provider.user.name} defaultAddress={user.locationLabel ?? ""} startingPrice={startingPrice} />
          </aside>
        </div>
      </div>
    </main>
